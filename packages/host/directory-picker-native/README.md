...
- **Non-Electron Windows hosts have no mechanism fallback** — outside the desktop parent's explicit IPC opt-in, the child-process picker through packaged koffi is the only native tier, so a COM refusal or dialog crash surfaces the failure. The browse backend remains the fallback at the composition level.
