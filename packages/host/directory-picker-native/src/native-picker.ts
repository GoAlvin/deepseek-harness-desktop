...
import { desktopParentPickerAvailable, pickDesktopParentDirectory } from './desktop-parent-picker.ts'
...
export interface DirectoryPickerInternals {
  ...
  /** Replaces the Electron-parent picker for deterministic tests. */
  pickDesktopParentDialog?: (signal: AbortSignal) => Promise<string | null>
  /** Overrides desktop-parent availability. */
  desktopParentPicker?: boolean
}
...
export async function pickNativeDirectory(
  ...
  if (platform === 'win32') {
    const useDesktopParent = internals.desktopParentPicker ?? desktopParentPickerAvailable()
    if (useDesktopParent) {
      const pickDesktopDialog = internals.pickDesktopParentDialog ?? pickDesktopParentDirectory
      return await pickDesktopDialog(signal)
    }
