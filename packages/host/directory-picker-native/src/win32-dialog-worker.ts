...
import { postWorkerMessage, type Win32DialogWorkerIpc } from './win32-dialog-worker-messenger.ts'
...
const ipc: Win32DialogWorkerIpc = {
...
void (async () => {
  try {
    ...
    const path = runFolderDialog(bindings, title, (threadId) => {
      postWorkerMessage(ipc, { kind: 'showing', threadId } satisfies Win32DialogWorkerMessage)
    })
    postWorkerMessage(ipc, { kind: 'done', path } satisfies Win32DialogWorkerMessage)
  } catch (error: unknown) {
    const message = error instanceof Error ? (error.stack ?? error.message) : String(error)
    postWorkerMessage(ipc, { kind: 'error', message } satisfies Win32DialogWorkerMessage)
  }
})()
