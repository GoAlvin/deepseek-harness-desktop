...
describe('native directory picker', () => {
  ...
  it('uses the Win32 dialog and never spawns a command when it answers', async () => {
    ...
    await expect(pickNativeDirectory(signal(), { platform: 'win32', run, pickWin32Dialog, desktopParentPicker: false })).resolves.toBe('C:\\work\\selected')
    pickWin32Dialog.mockResolvedValueOnce(null)
    await expect(pickNativeDirectory(signal(), { platform: 'win32', run, pickWin32Dialog, desktopParentPicker: false })).resolves.toBeNull()
    expect(run).not.toHaveBeenCalled()
  })

  it('prefers the Electron-parent dialog when the desktop host opts in', async () => {
    const run = vi.fn<DirectoryPickerRunner>()
    const pickWin32Dialog = vi.fn(async (): Promise<string | null> => 'C:\\worker')
    const pickDesktopParentDialog = vi.fn(async (): Promise<string | null> => 'C:\\electron')
    await expect(pickNativeDirectory(signal(), {
      platform: 'win32', run, pickWin32Dialog, pickDesktopParentDialog, desktopParentPicker: true,
    })).resolves.toBe('C:\\electron')
    expect(pickDesktopParentDialog).toHaveBeenCalledOnce()
  ...
  it('surfaces the Win32 dialog failure with no fallback', async () => {
    const run = vi.fn<DirectoryPickerRunner>()
    await expect(pickNativeDirectory(signal(), { platform: 'win32', run, pickWin32Dialog: noDialog, desktopParentPicker: false }))
  ...
  it('wires the real Win32 dialog as the default tier', async () => {
    ...
    abort.abort()
    const run = vi.fn<DirectoryPickerRunner>()
    await expect(pickNativeDirectory(abort.signal, { platform: 'win32', run, desktopParentPicker: false }))
  ...
  it('does not fall back when the caller aborted the dialog', async () => {
    ...
    const run = vi.fn<DirectoryPickerRunner>()
    await expect(pickNativeDirectory(abort.signal, { platform: 'win32', run, pickWin32Dialog: noDialog, desktopParentPicker: false })).rejects.toThrow('dialog unavailable')
    expect(run).not.toHaveBeenCalled()
  })
  ...
  it('uses the current process platform when no platform override is supplied', async () => {
    ...
    await expect(pickNativeDirectory(signal(), { run, pickWin32Dialog, desktopParentPicker: false })).resolves.toBe(expected)
  })
